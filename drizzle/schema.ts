...
export const fceApplications = pgTable("fce_applications", {
	...
	return {
		...
		fceApplicationsOfficeCheck: check("fce_applications_office_check", sql`office = ANY (ARRAY['boston'::text, 'new york'::text, 'miami'::text, 'san francisco'::text, 'los angeles'::text, NULL::text])`),
		fceApplicationsPaymentStatusCheck: check("fce_applications_payment_status_check", sql`payment_status = ANY (ARRAY['pending'::text, 'paid'::text, 'failed'::text, 'expired'::text])`),
